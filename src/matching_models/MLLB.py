...
class MLLB(object):
    """A Makespan paper matcher with a lower bound on reviewer load.
    ...
    def __init__(self, loads, l_loads, coverages, weights, makespan=0, gap=0.0):
        """Initialize a makespan matcher

        Args:
            loads - a list of integers specifying the maximum number of papers
                  for each reviewer.
            l_loads - a list of integers specifying the minimum number of papers
                  for each reviewer.
        ...
        for r, load in enumerate(self.loads):
            self.m.addConstr(sum(self.lp_vars[r]) <= load, "r_ub" + str(r))
        for r, l_load in enumerate(self.l_loads):
            self.m.addConstr(sum(self.lp_vars[r]) >= l_load, "r_lb" + str(r))
