...
class RelaxRevPaPMatcher(object):
    ...
    def round_fractional(self, integral_assignments, log_file=None):
        ...
        if self.integral_sol_found():
            ...
            for i in range(self.n_rev):
                if i not in fraction_rev_assign:
                    fraction_rev_assign[i] = []

                for j in range(self.n_pap):

                    if j not in fraction_pap_assign:
                        fraction_pap_assign[j] = []
                    ...
                    if sol[self.var_name(i,j)] == 0.0 and integral_assignments[i][j] != 0.0:
                        ...
                        fraction_pap_assign[j].append((i,j, sol[self.var_name(i,j)]))
                        fraction_rev_assign[i].append((i,j, sol[self.var_name(i,j)]))
            ...
            constrs_to_remove = {}
            for (paper, frac_vars) in fraction_pap_assign.iteritems():
                if len(frac_vars) == 1:
                    i,j,v = frac_vars[0]
                    integral_assignments[i][j] = 0.0
            ...
            for (rev, frac_vars) in fraction_rev_assign.iteritems():
                if len(frac_vars) == 2:
                    i,j1,v1 = frac_vars[0]
                    i,j2,v2 = frac_vars[1]

                    for c in self.m.getConstrs():
                        if c.ConstrName == self.rev_constr_prefix + str(i):
                            self.m.remove(c)
                            self.m.update()
                            return self.round_fractional(integral_assignments, log_file)
...
if __name__ == "__main__":
    ...
    print "running"
    x = RelaxRevPaPMatcher(n_rev, n_pap, alpha, beta, weights, init_makespan)
    x.solve()
